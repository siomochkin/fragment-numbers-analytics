...
url1 = 'https://fragment.com/numbers?sort=ending&filter=sold'
...
while True:   
    ...
    # Find number status
    status_find = str(soup1)[str(soup1).find('status-unavail'):]
    ...
    # Find sold time
    data_find = str(soup1)[str(soup1).find('datetime='):]
    ...
    # Convert data into datetime oblect and format it
    data_datetime = datetime.strptime(data, '%d %b %Y %H:%M')
    formatted_data = data_datetime.strftime('%Y-%m-%d %H:%M')

    with open('sold.csv', 'a+') as file:
        values = (formatted_data, str(price), str(ton), str(phone), str(status))
